...
% * Isso está sendo feito aqui para nós não nos perdemos ao escrever o restante do documento
%-----
<<label=Rsetup,echo=F,results=hide>>=
table <- read.table("studentdata.txt", head=T)
@
...
Escolhemos o conjunto de dados studentdata, do pacote LearnBayes do programa R. Abaixo, uma pequena amostra dos dados da tabela\footnote{Todos os dados da tabela, que possui 657 registros, podem ser encontrados no arquivo \textit{studentdata.txt}, que foi entregue junto com este relatório}:
